...
func FindUserByAccount(account string) (*TUsers, error){
	model := &TUsers{Account: account}
	has, err := database.Get(model)
	if err != nil {
		return nil, err
...
func FindUserByUserId(userId int64) (*TUsers,error){
...
func FindMessage(typ string,version string) (*TMessage, error){
	model := &TMessage{}
	model.Type = typ
	if version != "null" {
...
func FindRoomByRoomId(roomid string) (*TRooms, error){
	model := new(TRooms)
...
func FindRoomByServerId(serverId int64) ([]*TRooms){
